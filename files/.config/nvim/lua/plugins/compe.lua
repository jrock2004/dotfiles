...
require'compe'.setup {
  ...
  source = {
    path = {kind = icons.threeDots},
    buffer = {kind = icons.fileCutCorner},
    calc = {kind = icons.calculator},
    vsnip = {kind = icons.emptyBox, priority = 1},
    nvim_lsp = {kind = icons.hexCutOut},
    nvim_lua = {kind = icons.vim},
    -- nvim_lua = false,
    spell = {kind = icons.pencil},
    tags = true,
    treesitter = {icons.tree},
    emoji = {kind = icons.happyFace, filetypes={"markdown"}}
  }
}
...
imap("<CR>", "compe#confirm('<CR>')", options)
